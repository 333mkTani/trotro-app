...
export async function initializeLocalSync(): Promise<void> {
  ...
  await db.execAsync(`
    ...
    CREATE INDEX IF NOT EXISTS sync_queue_ready_idx
      ON sync_queue(user_id, status, id, created_at);
  ...
  if (!version) {
    await db.runAsync('INSERT INTO sync_meta (key, value) VALUES (?, ?)', 'schema_version', String(SCHEMA_VERSION));
  }
  initialized = true;
}
...
export async function listQueuedMutations(userId: string): Promise<QueuedMutation[]> {
  ...
  const rows = await db.getAllAsync<Record<string, unknown>>(
    `SELECT * FROM sync_queue
      WHERE user_id = ? AND status IN ('pending', 'conflict')
      ORDER BY id ASC
      LIMIT 100`,
    userId,
  );
...
export async function getCachedRecords(userId: string, entity: string): Promise<Array<Record<string, unknown>>> {
  ...
  const rows = await db.getAllAsync<Record<string, unknown>>(
    `SELECT entity_id, operation, payload, server_sequence, updated_at
       FROM sync_cache WHERE user_id = ? AND entity = ? ORDER BY updated_at DESC`,
    userId,
    entity,
  );
  return rows.map((row) => ({
